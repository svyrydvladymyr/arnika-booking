...
if ($outp != []){
    $stmts = $conn->prepare("SELECT last_name, first_name, nomer_kimn, data_zaizdu, telephone, kilk_dniv, price, tip, status FROM bookinggurt1 WHERE data_zaizdu >= ? AND data_zaizdu <= ? AND status = ?");
