...
if ($outp != []){
    $stmts = $conn->prepare("SELECT last_name, first_name, nomer_kimn, telephone, kilk_dniv, data_zaizdu, price, admin, data_zapisu, tip, admin_updata, data_zmin, status FROM bookinggurt2 WHERE last_name = ? AND first_name = ? AND nomer_kimn = ? AND telephone = ? AND kilk_dniv = ? AND (status = 'pay' OR status = 'rezerv')");
