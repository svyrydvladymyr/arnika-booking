...
if ($outp != []){
    $stmts = $conn->prepare("UPDATE bookinggurt2 SET status = ?, admin_updata = ?, data_zmin = ? WHERE last_name = ? AND first_name = ? AND telephone = ? AND nomer_kimn = ? AND kilk_dniv = ? AND data_zapisu = ? AND (status = 'pay' OR status = 'rezerv')");
